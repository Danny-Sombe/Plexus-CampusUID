...
**API base resolution** is a repeated idiom across every frontend JS file (`script.js`, `signup.js`, `pass.js`, `dashboard.js`, `student.html`, `code.html`). The pattern: use `localStorage.getItem('serverHost')` if set, else `http://localhost:5000` when the page is opened via `file://`, else the current page origin (with `:5000` appended). When adding a new page that calls the backend, copy this block so the page works both when served by Express and when opened as a file.
